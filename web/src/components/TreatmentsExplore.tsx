...
import {
  ...
  type ExploreTone,
} from "@/lib/treatmentsExplore";

type Props = {
  /** Controlled active tone from parent page */
  activeTone?: ExploreTone;
  onToneChange?: (tone: ExploreTone) => void;
  /** @deprecated use onToneChange */
  onNovimidChange?: (novimidId: "semaglutide" | "tirzepatide") => void;
};

/** Treatments explore — tabs + split hero */
export function TreatmentsExplore({
  activeTone,
  onToneChange,
  onNovimidChange,
}: Props) {
  const [internal, setInternal] = useState<ExploreTone>("semaglutide");
  const active = activeTone ?? internal;
  const activeIndex = exploreCategories.findIndex((c) => c.id === active);
  const cat =
    exploreCategories.find((c) => c.id === active) ?? exploreCategories[0];

  function select(next: ExploreCategory) {
  ...
  return (
    <div className="treatments-explore explore-tabs-wrap mx-auto w-full max-w-[72rem]">
      <div
      ...
      <ExplorePane key={cat.id} cat={cat} />
    </div>
  );
}

function ExplorePane({ cat }: { cat: ExploreCategory }) {
